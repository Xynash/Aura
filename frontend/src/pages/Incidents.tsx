...
import { 
  ...
  Database, Timer, ExternalLink, Radio
} from 'lucide-react';
...
// ── WebSocket hook (same as Dashboard) ────────────────────────────────────────
function useAuraSocket(onEvent: (e: AuraEvent) => void) {
  ...
  const connect = useCallback(() => {
    try {
      ...
      ws.onopen    = () => console.log("🔌 Incidents WS: connected.");
      ws.onmessage = (msg) => {
        try { onEventRef.current(JSON.parse(msg.data) as AuraEvent); }
        catch (e) { console.error("WS parse", e); }
      };
      ws.onclose = () => setTimeout(connect, 5000);
      ws.onerror = () => ws.close();
      wsRef.current = ws;
    } catch { console.warn("WS unavailable"); }
  }, []);
...
const Incidents = () => {
  ...
  // New: is the watcher pushing live data to this page?
  const [isLiveMode,         setIsLiveMode]         = useState(false);
  const [liveStatusText,     setLiveStatusText]     = useState('');

  // ── WebSocket: receive autonomous RCA from watcher ─────────────────────────
  useAuraSocket((event) => {
    if (event.type === "incident_detected") {
      // A real pod died — prime the UI immediately
      setIsLiveMode(true);
      setIsErrorActive(true);
      setAnalysisStep(2); // skip to "analyzing" state
    ...
    if (event.type === "rca_complete") {
      // AI finished — populate the full report automatically
      setIsLiveMode(true);
      setAnalysisData({
        pod:                event.pod,
      ...
      setAnalysisStep(4);
      setLiveStatusText(`RCA complete · node ${event.active_node}`);
      setChatMessages(prev => [...prev, {
        type: 'bot',
        text: `Neural link via ${event.active_node}. Autonomous analysis complete. Source-aware patch generated.`
      }]);
    }
  });

  // ── Manual investigation (simulation mode — unchanged) ─────────────────────
  ...
  useEffect(() => {
    // If Dashboard already stored a completed autonomous RCA, render it instantly
    ...
    if (autoRCA) {
      try {
        ...
        setAnalysisStep(4);
        setLiveStatusText(`Autonomous RCA loaded · pod ${parsed.pod}`);
        setChatMessages([{ type: 'bot', text: `Neural link via ${parsed.active_node}. Autonomous analysis complete.` }]);
        return;
                  ...
                  <div>
                    ...
                    {isLiveMode && (
                      <p className="text-[9px] text-red-400 font-mono mt-1 uppercase tracking-widest">
                        Zero human intervention · watcher triggered
                      </p>
                    )}
                  </div>
                </div>
            ...
            {analysisStep >= 4 && (
              <div className="grid grid-cols-1 gap-6">
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="aura-card-modern p-0 overflow-hidden border-[#bef35e]/20 group"
                >
                  <div className="bg-zinc-900/80 px-8 py-6 border-b border-white/5 flex justify-between items-center font-mono text-[10px]">
                    <span className="font-black text-zinc-400 uppercase flex items-center gap-2">
                      <Code size={14} /> Source: AuthService.java
                    </span>
