      ...
      <AnimatePresence>
        {isAutoAnalyzing && (
          ...
          >
            ...
            <div>
              <p className="text-red-400 text-[10px] font-black uppercase tracking-widest">Autonomous Detection Active</p>
              <p className="text-white text-xs font-mono">Pod <span className="text-red-400">{activeService}</span> · AI analyzing in real-time</p>
            </div>
            <Loader2 className="text-red-400 animate-spin ml-2" size={16} />
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className={`col-span-12 lg:col-span-8 aura-card-modern p-10 min-h-[550px] relative overflow-hidden group border-white/5 bg-black/40 transition-all duration-700 ${isIncidentActive ? 'border-red-500/20' : ''}`}>
          ...
          <div className="absolute bottom-10 left-10 right-10 flex justify-between items-end z-20">
            {!isIncidentActive ? (
              <div className="space-y-3">
                <button
                  ...
                  className="bg-white/5 border border-white/10 px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all shadow-2xl"
                >
              ...
              <div className="space-y-1">
                <p className="text-[10px] font-black text-red-500 uppercase tracking-[0.4em]">
                  {isAutoAnalyzing ? 'Autonomous_RCA_Running' : 'Active_Incident_Report'}
                </p>
            ...
            <motion.div
              ...
              className={`p-4 rounded-2xl ${isIncidentActive ? 'bg-red-500 text-white shadow-red-500/30' : 'bg-white/5 text-[#bef35e]'} shadow-xl`}
            >
        ...
        <div className="col-span-12 lg:col-span-4 aura-card-modern p-8 flex flex-col border-white/5 bg-black/60">
          ...
          {isIncidentActive && (
            <button
              ...
              className="mt-10 bg-[#bef35e] text-black py-4 rounded-2xl font-black uppercase tracking-widest text-[11px] shadow-[0_10px_40px_rgba(190,243,94,0.3)] hover:scale-[1.02] transition-transform"
            >
              {isAutoAnalyzing ? 'Watch_Live_Analysis →' : 'Investigate_Incident_Lab'}
            </button>
          )}
        </div>

        {/* ── 4. AURA ENGINE PROTOCOL ── */}
...
const LogStep = ({ title, desc, active }: any) => (
  <div className={`flex gap-5 items-start ${active ? 'opacity-100' : 'opacity-20'}`}>
    ...
    <div className="pt-1">
      <h5 className="text-[11px] font-black uppercase text-white">{title}</h5>
      <p className="text-[10px] text-zinc-500 font-medium italic">{desc}</p>
    </div>
  </div>
);
